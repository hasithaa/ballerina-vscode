...
// Durable-agent register/add statements: rendered without the module prefix and with the
// registered name (metadata.description) as the node's second line.
...
export function getNodeTitle(node: FlowNode) {
    ...
    // Durable-agent register statements keep their plain label ("Register Event", ...) without
    // the module prefix; the registered name is rendered as the node's second line instead.
